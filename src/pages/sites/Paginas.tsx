...
const Paginas = () => {
    ...
    const handleUpdateSection = (updates: Partial<Section>) => {
        ...
        updatedSections[currentSectionIndex] = { ...currentSection, ...updates };

        setPages(
            pages.map((p) =>
                p.id === selectedPageId ? { ...p, sections: updatedSections } : p
            )
        );
    };
    ...
    const handleImageUpload = (
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                if (field === "imageUrl") {
                    handleUpdateSection({ imageUrl: reader.result as string });
                    ...
                    newCards[cardIndex] = { ...newCards[cardIndex], imageUrl: reader.result as string };
                    handleUpdateSection({ cards: newCards });
                }
            };
    ...
    const handleAddCard = () => {
        ...
        handleUpdateSection({ cards: [...currentSection.cards, newCard] });
    };
    ...
    const toggleItems: { id: keyof Section; label: string; icon: ElementType }[] = [
        { id: "hasTitle", label: "Título", icon: Type },
        { id: "hasSubtitle", label: "Subtítulo", icon: Type },
        { id: "hasDescription", label: "Descrição", icon: FileText },
        { id: "hasImage", label: "Imagem", icon: ImageIcon },
        { id: "hasCarousel", label: "Carrossel", icon: Layers },
        { id: "hasButton", label: "Botão", icon: MousePointerClick },
        { id: "hasCards", label: "Cards", icon: Layout },
    ];
            ...
            <DashboardLayout>
                <div className="p-4 lg:p-6 relative z-10 h-[calc(100vh-100px)] flex flex-col">
                                                        ...
                                                        <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
                                                            {section.carouselImages.map((img, idx) => (
                                                                <img
                                                                    ...
                                                                    src={img}
                                                                    alt={`Slide ${idx}`}
                                                                    className="h-64 w-auto rounded-lg shadow-md snap-center"
                                                                />
                                                            ))}
                                                        </div>
                                                    )}
                    ...
                    <div className="flex flex-1 gap-6 overflow-hidden relative">
                        {/* Collapsible Sidebar */}
                        <div
                            className="group absolute left-0 top-0 bottom-0 z-20 flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-300 ease-in-out w-[66px] hover:w-[280px] overflow-hidden hover:shadow-xl"
                        >
                            ...
                            <div className="flex-1 overflow-y-auto overflow-x-hidden p-2 space-y-1 scrollbar-thin scrollbar-thumb-gray-200">
                                {selectedPage.sections.map((section, index) => (
                                    <div
                                        ...
                                        className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-all relative group/item ${index === currentSectionIndex
                                        ...
                                        <div className="flex-1 min-w-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-between">
                                            ...
                                            {selectedPage.sections.length > 1 && (
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    className="h-6 w-6 opacity-0 group-hover/item:opacity-100 hover:bg-red-100 hover:text-red-600"
                                ...
                                <div className="flex items-center justify-between mb-4 shrink-0 px-1">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            size="icon"
                                            className="h-8 w-8 bg-white"
                                            disabled={currentSectionIndex === selectedPage.sections.length - 1}
                                            onClick={() => setCurrentSectionIndex((prev) => prev + 1)}
                                        >
                                    ...
                                    <div className="flex-1 overflow-y-auto pr-2 pb-10">
                                        <div className="grid gap-6 max-w-6xl mx-auto">
                                                ...
                                                <CardContent className="pt-6">
                                                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                                                        {toggleItems.map((item) => (
                                                            <div
                                                                key={item.id}
                                                                className={`flex flex-col gap-3 p-3 rounded-xl border transition-all cursor-pointer ${currentSection[item.id]
                                                                    ? "bg-blue-50 border-blue-200 shadow-sm"
                                                                    : "bg-white border-slate-100 hover:border-slate-300"
                                                                    }`}
                                                                onClick={() => handleUpdateSection({ [item.id]: !currentSection[item.id] })}
                                                            >
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <Switch
                                                                        id={item.id}
                                                                        checked={currentSection[item.id] as boolean}
                                                                        onCheckedChange={(checked) => handleUpdateSection({ [item.id]: checked })}
                                            ...
                                            <Accordion type="multiple" defaultValue={["content", "media"]} className="space-y-4">
                                                {/* Conteúdo de Texto */}
                                                {(currentSection.hasTitle || currentSection.hasSubtitle || currentSection.hasDescription) && (
                                                    <AccordionItem value="content" className="border rounded-xl bg-white px-4 shadow-sm">
                                                ...
                                                {/* Mídia */}
                                                {(currentSection.hasImage || currentSection.hasCarousel) && (
                                                    <AccordionItem value="media" className="border rounded-xl bg-white px-4 shadow-sm">
                                                        <AccordionTrigger className="hover:no-underline py-4">
                                                            <div className="flex items-center gap-2 text-slate-800">
                                                        ...
                                                        <AccordionContent className="space-y-6 pb-4">
                                                            {currentSection.hasImage && (
                                                                <div className="p-4 rounded-lg border border-dashed border-slate-300 bg-slate-50/50">
                                                            ...
                                                            {currentSection.hasCarousel && (
                                                                <div className="space-y-3">
                                                                    ...
                                                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4 rounded-lg bg-slate-50 border min-h-[120px]">
                                                                        {currentSection.carouselImages.map((img, idx) => (
                                                                            <div key={idx} className="relative group aspect-video rounded-lg overflow-hidden bg-white shadow-sm border">
                                                                                <img
                                                                                    src={img}
                                                                                    alt={`Carousel ${idx}`}
                                                                                    className="h-full w-full object-cover"
                                                                                ...
                                                                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                                                    <Button
                                                                                        ...
                                                                                        onClick={() => {
                                                                                            ...
                                                                                            handleUpdateSection({ carouselImages: newImages });
                                                                                        }}
                                                                                    >
                                                ...
                                                {/* Cards Editor */}
                                                {currentSection.hasCards && (
                                                    <AccordionItem value="cards" className="border rounded-xl bg-white px-4 shadow-sm">
                                                        <AccordionTrigger className="hover:no-underline py-4">
                                                            <div className="flex items-center gap-2 text-slate-800">
                                                                            ...
                                                                            <div className="space-y-2">
                                                                                ...
                                                                                <div className="relative">
                                                                                    ...
                                                                                    <Button
                                                                                        ...
                                                                                        onClick={() => document.getElementById(`card-img-${card.id}`)?.click()}
                                                                                    >
                                                                                        Alterar
                                                                                    </Button>
                                                                                </div>
                                                                            ...
                                                                            <div className="space-y-3">
                                                                                ...
                                                                                <div className="flex gap-2">
                                                                                    <Input
                                                                                        ...
                                                                                        onChange={(e) => handleUpdateCard(index, { buttonText: e.target.value })}
                                                                                        placeholder="Texto do botão"
                                                                                        className="flex-1 text-sm"
                                                                                    />
                                                                                    <Input
                                                                                        value={card.buttonLink}
                                                                                        onChange={(e) => handleUpdateCard(index, { buttonLink: e.target.value })}
                                                                                        placeholder="Link"
                                                                                        className="flex-1 text-sm"
                                                                                    />
                                                                                </div>
                                                ...
                                                {currentSection.hasButton && (
                                                    <AccordionItem value="actions" className="border rounded-xl bg-white px-4 shadow-sm">
                                                        <AccordionTrigger className="hover:no-underline py-4">
                                                            <div className="flex items-center gap-2 text-slate-800">
                                                    ...
                                                    <AccordionContent className="pb-4">
                                                        <div className="space-y-2">
                                                            ...
                                                            <Input
                                                                ...
                                                                onChange={(e) => handleUpdateSection({ id: e.target.value })}
